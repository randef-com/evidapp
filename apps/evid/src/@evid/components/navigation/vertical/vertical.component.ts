...
import { EvidNavigationItem, EvidVerticalNavigationAppearance, EvidVerticalNavigationMode, EvidVerticalNavigationPosition } from '../navigation.types';
import { EvidNavigationService } from '../navigation.service';
import {EvidScrollbarDirective} from "../../../directives/scrollbar";
import {EvidAnimations} from "../../../animations";

@Component({
    selector       : 'evid-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : EvidAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'EvidVerticalNavigation'
})
export class EvidVerticalNavigationComponent implements OnInit, AfterViewInit, OnDestroy
{
    activeAsideItemId: null | string;
    onCollapsableItemCollapsed: BehaviorSubject<EvidNavigationItem | null>;
    onCollapsableItemExpanded: BehaviorSubject<EvidNavigationItem | null>;
    ...
    @Output()
    readonly appearanceChanged: EventEmitter<EvidVerticalNavigationAppearance>;

    // On mode changed
    @Output()
    readonly modeChanged: EventEmitter<EvidVerticalNavigationMode>;
    ...
    // On position changed
    @Output()
    readonly positionChanged: EventEmitter<EvidVerticalNavigationPosition>;

    // Private
    private _appearance: EvidVerticalNavigationAppearance;
    ...
    private _mode: EvidVerticalNavigationMode;
    private _navigation: EvidNavigationItem[];
    private _opened: boolean | '';
    private _overlay: HTMLElement | null;
    private _player: AnimationPlayer;
    private _position: EvidVerticalNavigationPosition;
    private _scrollStrategy: ScrollStrategy;
    private _transparentOverlay: boolean | '';
    private _EvidScrollbarDirectives: QueryList<EvidScrollbarDirective>;
    private _EvidScrollbarDirectivesSubscription: Subscription;
    private _unsubscribeAll: Subject<any>;

    @HostBinding('class.evid-vertical-navigation-animations-enabled')
    ...
    )
    {
        ...
        // Set the defaults
        this.appearanceChanged = new EventEmitter<EvidVerticalNavigationAppearance>();
        this.modeChanged = new EventEmitter<EvidVerticalNavigationMode>();
        this.openedChanged = new EventEmitter<boolean | ''>();
        this.positionChanged = new EventEmitter<EvidVerticalNavigationPosition>();
     ...
     */
    @Input()
    set appearance(value: EvidVerticalNavigationAppearance)
    {
        ...
        appearanceClassName = 'evid-vertical-navigation-appearance-' + this.appearance;
    ...
    get appearance(): EvidVerticalNavigationAppearance
    {
        return this._appearance;
    }

    /**
     * Setter for EvidScrollbarDirectives
     */
    @ViewChildren(EvidScrollbarDirective)
    set EvidScrollbarDirectives(EvidScrollbarDirectives: QueryList<EvidScrollbarDirective>)
    {
        // Store the directives
        this._EvidScrollbarDirectives = EvidScrollbarDirectives;

        // Return, if there are no directives
        if ( EvidScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._EvidScrollbarDirectivesSubscription )
        {
            this._EvidScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._EvidScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    EvidScrollbarDirectives.forEach((EvidScrollbarDirective) => {
                        EvidScrollbarDirective.update();
                    });
                });
    }
    ...
    set inner(value: boolean)
    {
        ...
        if ( this.inner )
        {
            this._renderer2.addClass(this._elementRef.nativeElement, 'evid-vertical-navigation-inner');
        }
        else
        {
            this._renderer2.removeClass(this._elementRef.nativeElement, 'evid-vertical-navigation-inner');
        }
    }
     ...
     */
    @Input()
    set mode(value: EvidVerticalNavigationMode)
    {
        // If the value is the same, return...
        if ( this._mode === value )
        {
        ...
        modeClassName = 'evid-vertical-navigation-mode-' + this.mode;
    ...
    get mode(): EvidVerticalNavigationMode
    {
        return this._mode;
    }
    ...
    set opened(value: boolean | '')
    {
        ...
        if ( this.opened )
        {
            ...
            this._renderer2.addClass(this._elementRef.nativeElement, 'evid-vertical-navigation-opened');
        }
        else
        {
            // Update styles and classes
            this._renderer2.setStyle(this._elementRef.nativeElement, 'visibility', 'hidden');
            this._renderer2.removeClass(this._elementRef.nativeElement, 'evid-vertical-navigation-opened');
        }
     ...
     */
    @Input()
    set position(value: EvidVerticalNavigationPosition)
    {
        // If the value is the same, return...
        ...
        positionClassName = 'evid-vertical-navigation-position-' + this.position;
    ...
    get position(): EvidVerticalNavigationPosition
    {
    ...
    ngAfterViewInit(): void
    {
        setTimeout(() => {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.evid-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._EvidScrollbarDirectives.forEach((EvidScrollbarDirective) => {

                    // Skip if not enabled
                    if ( !EvidScrollbarDirective.enabled )
                    {
                        return;
                    }

                    // Scroll to the active element
                    EvidScrollbarDirective.scrollToElement('.evid-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('evid-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('evid-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('evid-vertical-navigation-aside-overlay');
    ...
    private _onMouseenter(): void
    {
        ...
        // Add a class
        this._renderer2.addClass(this._elementRef.nativeElement, 'evid-vertical-navigation-hover');
    }
    ...
    private _onMouseleave(): void
    {
        ...
        // Remove the class
        this._renderer2.removeClass(this._elementRef.nativeElement, 'evid-vertical-navigation-hover');
    }
     ...
     * @param item
     */
    openAside(item: EvidNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: EvidNavigationItem): void
    {
        // Toggle
