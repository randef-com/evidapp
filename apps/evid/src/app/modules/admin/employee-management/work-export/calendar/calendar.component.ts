...
export class CalendarComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  onDateClick(calendarEvent): void {
    const event = {
      ...
      title: '',
      description: '',
      start: moment(calendarEvent.date).hour(8).minute(30).toISOString(),
      end: moment(calendarEvent.date).hour(17).toISOString(),
      duration: null,
      allDay: false,
      recurrence: null,
      range: {
  ...
  onEventRender(calendarEvent): void {
    ...
    if (this.view === 'listYear') {
      ...
      // Set the event's title to calendars title  if event title is not available
      if (!calendarEvent.event.title) {
        calendarEvent.el.querySelector('.fc-list-item-title').innerText = calendar.title;
      }
    }
    // If current view is not month list...
      ...
      if (!calendarEvent.event.title) {
        calendarEvent.el.querySelector('.fc-title').innerText = calendar.title;
      }
    }
